...
const char *configLoadErrorToReason(ConfigLoadError error)
{
  switch (error)
  {
  case ConfigLoadError::None:
    return "none";
  case ConfigLoadError::LittleFsMountFailed:
    return "littlefs_mount_failed";
  ...
  case ConfigLoadError::InvalidSecurityMaterial:
    return "invalid_security_material";
  default:
    return "unknown";
  }
}
...
bool loadSystemConfig(const char *path)
{
  ...
  if (!LittleFS.begin())
  {
    lastConfigLoadError = ConfigLoadError::LittleFsMountFailed;
    return false;
  }
  ...
  if (!file)
  {
    lastConfigLoadError = ConfigLoadError::ConfigOpenFailed;
    return false;
  }
  ...
  {
    lastConfigLoadError = ConfigLoadError::MissingRequiredSection;
    return false;
  }
  ...
  if (!ok)
  {
    lastConfigLoadError = ConfigLoadError::InvalidSecurityMaterial;
    return false;
  }
